...
const apiDomainUrl = "https://harbour1.in/partnerportal_backend/";
...
function fillReviewPage(partnerObj) {
  if (partnerObj.status === PENDING_STATE) {
    if ($("#page-single-partner #row-cta").hasClass("hide")) {
      $("#page-single-partner #row-cta").removeClass("hide");
    }
  } else {
    if (!$("#page-single-partner #row-cta").hasClass("hide")) {
      $("#page-single-partner #row-cta").addClass("hide");
    }
  }
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.status == "success") {
        ...
        snackbar("Error in getting data");
        return false;
      }
    },
    error: function (data) {
