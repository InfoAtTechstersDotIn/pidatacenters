...
*/
	public function pipelinesByParams(Request $request) {
        	// $startDate = "2021-05-01";
        	// $endDate = "2021-08-12";
        	$param1 = $request->fc1;
        	$param2 = $request->fc2;
        	...
        	switch ($param1) {
            		case "stages":  $stage = Stage::where("display_name", '=', $param2)->first();
                            $filterId = ($stage == null)?0 :$stage->id;
                            $parentField = "stage";
                            $title = "Pipelines in " . $stage->display_name . " stage";
                            break;
            		case "customer-type": $custTypes = CustomerType::where("customer_type_name", '=', $param2)->first();
                            $filterId = ($custTypes == null)?0 : $custTypes->id;
                            $parentField = "customer_type";
                            $title = "Pipelines in " . $custTypes->customer_type_name . " customers";
                            break;
            		case "industry": $industryTypes = BusinessIndustry::where("display_value", '=', $param2)->first();
                            $filterId = ($industryTypes == null)?0 : $industryTypes->id;
                            $parentField = "business_industry";
                            $title = "Pipelines in " . $industryTypes->display_name . " industry";
                            break;
            		case "sbu":     $sbu = SBUnit::where("sbu_name", '=', $param2)->first();
                            $filterId = ($sbu == null)?0 : $sbu->id;
                            $parentField = "sbunits_id";
                            $title = "Pipelines in " . $sbu->sbu_name . " business unit";
                            break;

            		case "rating":  $rating = PipelineRating::where("rate_name", "LIKE", $param2." - %")->first();
                            $filterId = ($rating == null)?0 : $rating->id;
                            $parentField = "rating_id";
                            $title = "Pipelines in " . $rating->rate_name . " rating";
                            break;
