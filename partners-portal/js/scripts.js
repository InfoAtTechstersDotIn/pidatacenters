    ...
    if ($(this).hasClass('tab_next') == true) {
        if (validatepersoninfo() == true) {
            if ($('#p_bemail').val()) {
                ...
                $.ajax({
                    url: "https://partnerportal.pidatacenters.com/slimportal/public/partner/partnermailcheck",
                    type: "POST",
                    data: input,
                    crossDomain: true,
                    dataType: 'json',
                    ...
                    success: function (data) {
                        console.log("mallikarjuna aaaaaaaaaaa sucess" + data);
                        $('#personal_header').tab('show');
...
$(document).on('click focus', '#o_cspr_sind', function () {
...
var titledrpdwn, p_fname, p_lname, p_jobtitle, p_bemail, phone, extension, o_cname, drpDwnTxt_city, o_cwebsite, o_chq, o_czipcode, o_cdesc, countrydd, o_cspr, empstrength, o_arevenue, o_indvertical, lob;
...
function validatepersoninfo() {
    ...
    if (titledrpdwn == "Select" || p_fname == '' || p_lname == '' || p_jobtitle == '' || p_bemail == '' || phone == '') {
        ...
        return false;
    } else if (String(bem.trim()).toLowerCase() == 'gmail.com' ||
...
function validateorganisationinfo() {
    ...
    if (document.getElementById('lob').value.trim()=="Others" && lob_other_input == '') {
        snackbar('Please Enter Other Line Of Business');
        return false;
    }
    if (o_cname == '' || o_cwebsite == '' || o_chq == '' || o_czipcode == '' || o_cdesc == '' || countrydd == '' || empstrength == 'Select' || o_arevenue == '' || o_indvertical == 'Select' || lob == 'Select' || o_cspr == '' || o_cspr == '') {
        snackbar('All Fields Are Mandatory');
        return false;
    } else if (o_arevenue.replace(/^0+/, '') == '') {
        snackbar('Annual Revenue Could Never Be 0');
        return false;
    } else if (subselectedArray_usa.length == 0 && subselectedArray_india.length == 0 && selectedArray1.length == 0) {
        snackbar('All Fields Are Mandatory');
        return false;
    } else if (!re.test(o_cwebsite)) {
        snackbar('Please Enter A Valid Website');
        return false;
    } else if (checkinputchecked('tar_cli_base') && !(document.getElementById('tc_otherr').checked)) {
...
function validatetechnicalinfo() {
    ...
    if (checkinputchecked('u_case_expe') && !(document.getElementById('uce_otherr').checked)) {
        snackbar('Please Choose Atleast One Use Case Expertise');
        return false;
    } else if (checkinputchecked('tech_expe') && !(document.getElementById('te_othh').checked)) {
        snackbar('Please Choose Atleast One Technology Expertise');
        return false;
    } else if (checkinputchecked('prof_ser_off') && !(document.getElementById('pso_othee').checked) ) {
        snackbar('Please Choose Atleast One Professional Services Offered');
        return false;
    } else if (checkinputchecked('int_ser_prov') && !(document.getElementById('isp_othh').checked)) {
...
$('#send_form').click(function (e) {
    ...
    if (b_brieftxt == '' || partnership_model == undefined) {
        ...
        console.log(JSON.stringify(input));
        $.ajax({
            url: "https://partnerportal.pidatacenters.com/slimportal/public/partner/signup",
            type: "POST",
            data: input,
            crossDomain: true,
            ...
            ContentType: 'application/json; charset=utf-8',
            success: function (data) {
                console.log("send formmmmmm sucess if" + data);
                $('#loader').hide();
                ...
                if (resultError == "Success" || resultError == "success") {
                    ...
                    $('#cntr_crd_fin').append('<img src="assets/images/Group149.png">');
                    $('#cntr_txt_fin').append('<div><h2>Thank you for your interest in partnering with <br>Pi.</h2> <p>Your registration is currently being reviewed.<br>You will soon have a revert from us, on your registered Email ID.</p><a class="btn btn-success" id="ok_submit_btn" href="#">&emsp; OK &emsp;</a></div>');
                    $('#cntr_crd_fin').addClass('cntr_crd_fin');
                    $('#cntr_txt_fin div').addClass('cntr_crd_fin_anim');
                } else {
                    $('#loader').hide();
                    console.log("send formmmmmm error else" + data);
...
$(document).on('click', '#ok_submit_btn', function () {
    ...
    setTimeout(
        function () {
            // window.location.href = "https://pidatacenters.com/partner";
            window.location.href = "https://harbour1.in/";
            $('.loader').hide();
        }, 2000);
});


$('#loc_o_b').next().click(function (e) {
    e.stopPropagation();
});
...
$('.multi-level .dropdown-menu-item').click(function () {
    ...
    if ($(this).hasClass('active')) {
        ...
        selectedArray1.push('<span class="selectedValues_style"><a href="#">' + selecteditems + '</a></span>');
    } else {
        removeItemFunction(this);
    }
    selectedArray = selectedArray1;
    var countries_array = selectedArray.toString();
    document.getElementById("selectedValues").innerHTML = countries_array.replace(/\,/g, ' ');
...
$(".dropdown-submenu .dropdown-submenu-item").click(function () {
    ...
    if (selectedOption == "USA") {
        $(this).parent().parent().find('.ds-selected').text('USA').addClass('active');
        if ($(this).hasClass('active')) {
            $(this).hasClass('active').value = this.innerText;
            let selecteditems = this.innerText;
            let selecteditems11 = "USA - " + this.innerText;
            subselectedArray11_usa.push(selecteditems11);
            subselectedArray1_usa.push('<span class="selectedValues_style"><a href="#">' + " USA" + " - " + selecteditems + '</a></span>');
        } else {
        ...
        if (subselectedArray_usa.length >= 1) {
            ...
            document.getElementById("selectedValuesforUSA").innerHTML = usa_array.replace(/\,/g, ' ');
        } else {
    ...
    if (selectedOption == "India") {
        $(this).parent().parent().find('.ds-selected').text('India').addClass('active');
        if ($(this).hasClass('active')) {
            $(this).hasClass('active').value = this.innerText;
            let selecteditems = this.innerText;
            let selecteditems1 = "India - " + this.innerText;
            subselectedArray11_india.push(selecteditems1);
            subselectedArray1_india.push('<span class="selectedValues_style"><a href="#">' + " India" + " - " + selecteditems + '</a></span>');
        } else {
        ...
        if (subselectedArray_india.length >= 1) {
            ...
            document.getElementById("selectedValuesforIndia").innerHTML = india_array.replace(/\,/g, ' ');
        } else {
            finalselectedArray_india = "";
            document.getElementById("selectedValuesforIndia").innerHTML = finalselectedArray_india;
        }
    }
...
$(document).on('click', '#selectedValues .selectedValues_style', function () {
    ...
    $('#locofbusinees_dropdown li a:contains(' + $.trim(ItemTextVal) + ')').parent().removeClass('active');

    console.log('ItemTextVal : ' + ItemTextVal);
    let indexValue = selectedArray1.indexOf('<span class="selectedValues_style"><a href="#">' + ItemTextVal + '</a></span>')
    if (indexValue > -1) {
...
function removeItemFunction(e) {
    ...
    console.log(selectedArray1.indexOf('<span class="selectedValues_style"><a href="#">' + e.innerText + '</a></span>'));
    let indexValue = selectedArray1.indexOf('<span class="selectedValues_style"><a href="#">' + e.innerText + '</a></span>')
    if (indexValue > -1) {
        selectedArray1.splice(indexValue, 1);
    }
...
$(document).on('click', '#selectedValuesforUSA .selectedValues_style', function () {
    ...
    $('#locofbusinees_dropdown li ul li a:contains(' + $.trim(splitName[1]) + ')').parent().removeClass('active');
    ...
    let indexValue_usatext = subselectedArray1_usa.indexOf('<span class="selectedValues_style"><a href="#"> ' + ItemTextVal1 + '</a></span>');
...
function removeItemFunction_usa(e) {
    ...
    if ($('#locofbusinees_dropdown li a ul .dropdown-submenu-item').hasClass('active')) {
        $(e).parent().prev().removeClass('active');
    }
    $(e).hasClass('active').value = e.innerText;
    console.log('<span class="selectedValues_style"><a href="#">' + " USA" + " - " + e.innerText + '</a></span>');
    var indexValue_usa = subselectedArray1_usa.indexOf('<span class="selectedValues_style"><a href="#">' + " USA - " + e.innerText + '</a></span>');
    if (indexValue_usa > -1) {
...
$(document).on('click', '#selectedValuesforIndia .selectedValues_style', function () {
    ...
    $('#locofbusinees_dropdown li ul li a:contains(' + $.trim(splitName_india[1]) + ')').parent().removeClass('active');
    ...
    let indexValue_indiatext = subselectedArray1_india.indexOf('<span class="selectedValues_style"><a href="#"> ' + ItemTextVal2 + '</a></span>')
...
function removeItemFunction_india(e) {
    ...
    var indexValue_india = subselectedArray1_india.indexOf('<span class="selectedValues_style"><a href="#">' + " India - " + e.innerText + '</a></span>')
...
$('#indiastate_dropdown .dropdown-item').click(function () {
    ...
    $(this).parent().prev().focus();
    $.ajax({
        url: "https://partnerportal.pidatacenters.com/slimportal/public/getcity?state_name=" + drpDwnTxt,
        type: "get",
        crossDomain: true,
        // dataType: 'json',
        ContentType: 'application/json; charset=utf-8',
        success: function (data) {
            ...
            for (i = 0; i < selectedCities.length; i++) {
                var all_cities = '<a class="dropdown-item" href="#" >' + selectedCities[i] + '</a>';
...
$(document).on('click', '#city_dropdown .dropdown-item', function () {
    ...
    if (drpDwnTxt_city) {
        $.ajax({
            url: "https://partnerportal.pidatacenters.com/slimportal/public/getrolist?city_name=" + drpDwnTxt_city,
            type: "get",
            crossDomain: true,
            // dataType: 'json',
            ContentType: 'application/json; charset=utf-8',
